...
(function importWebAnimationsPolyfill() {
    // With synchonous Javascript execution we should be the last script tag at this point in time.
    var scripts = document.getElementsByTagName('script');
    var thisPath = scripts[scripts.length - 1].src;
    var importPath = thisPath.replace(/([^\/]+\/){2}testharnessreport.js$/, 'web-animations-next.dev.js');
    if (thisPath == importPath) {
        throw new Error("Failed to generate polyfill library path.");
    }
    document.write('<script src="' + importPath + '"></script>');
})();
